...
let DaysComponent = () => {
  ...
  return (
    <section className="container my-3 my-md-5 px-4">
      <h2>
        Days Weather{" "}
        <span className="text-sky text-capitalize">
          {name} , {country}{" "}
        </span>{" "}
      </h2>

      {dataFinalHolder.forecast
        ? dataFinalHolder.forecast.forecastday.map((d, index) => {
            if (dataFinalHolder.forecast && dataFinalHolder.location) {
              var { hour, date } = d;
            }
            return (
              <div>
                <h3 className="d-block mt-5 bg-dark text-white p-3">
                  Day {index + 1} - {date}
                </h3>
                        ...
                        return (
                          <div className="accordion-item" key={index}>
                            <h2 className="accordion-header" id={`w${index}`}>
                              <button
                                className="accordion-button fs-5 d-flex text-sky"
                                type="button"
                              ...
                              >
                                <FontAwesomeIcon
                                  icon={solidIcons.faBriefcaseClock}
                                  className="text-sky me-2"
                                />
                                <span className="me-3">
                                  {index++ + ` : 00`} {index < 13 ? "am" : "pm"}
                                </span>
                                <FontAwesomeIcon
                                  icon={solidIcons.faTemperature0}
                                  className="text-sky me-2"
                                />
                                  ...
                                  <div className="row">
                                    <div className="col-4 d-flex flex-nowrap justify-content-center">
                                      <div className="d-flex px-2">
                                        <FontAwesomeIcon
                                          icon={solidIcons.faTemperatureHalf}
                                          className="mt-1"
                                        />
                                      </div>
                                    ...
                                    <div className="col-4 d-flex flex-nowrap justify-content-center">
                                      <div className="d-flex px-2">
                                        ...
                                        <FontAwesomeIcon
                                          icon={solidIcons.faCloudShowersWater}
                                          className="mt-1"
                                        />
                                      </div>
                                      <div className="d-flex flex-column">
